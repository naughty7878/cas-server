...
class CasWebflowContextConfiguration {
    ...
    static class CasWebflowContextFlowExecutorConfiguration {
        ...
        public FlowExecutor logoutFlowExecutor(
                ...
                final CasConfigurationProperties casProperties,
                @Qualifier(CasWebflowConstants.BEAN_NAME_FLOW_DEFINITION_REGISTRY) final FlowDefinitionRegistry flowDefinitionRegistry,
                @Qualifier(CipherExecutor.BEAN_NAME_WEBFLOW_CIPHER_EXECUTOR) final CipherExecutor webflowCipherExecutor) {
    ...
    static class CasWebflowDefinitionsConfiguration {
        ...
        // 创建并初始化 Spring Web Flow 的流程定义注册中心（FlowDefinitionRegistry）
        // 登录/登出的 Webflow
        @Bean
        @RefreshScope(proxyMode = ScopedProxyMode.DEFAULT)
        public FlowDefinitionRegistry flowDefinitionRegistry(final ConfigurableApplicationContext applicationContext,
                                                             @Qualifier(CasWebflowConstants.BEAN_NAME_FLOW_BUILDER_SERVICES) final FlowBuilderServices flowBuilderServices,
